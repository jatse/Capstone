...
void frag_main()
{
    ...
    {
        _304 = octahedronWrap(g0.xy);
    }
    n = float3(_304.x, _304.y, n.z);
    n = normalize(n);
    ...
    float3 _417 = fragColor.xyz * ssaotex.SampleLevel(_ssaotex_sampler, texCoord, 0.0f).x;
    fragColor = float4(_417.x, _417.y, _417.z, fragColor.w);
    if (g0.w == 1.0f)
    {
        float3 _429 = fragColor.xyz + g1.xyz;
        fragColor = float4(_429.x, _429.y, _429.z, fragColor.w);
        albedo = 0.0f.xxx;
    }
    fragColor.w = 1.0f;
}
