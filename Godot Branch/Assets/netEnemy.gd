...
func _ready():
	...
	#determine if this is server instance
...
func _process(delta):
	...
	#have server decide action and sync across other peers
	if is_server:
		#ENEMY AI
		...
		#sync enemy
		rpc_unreliable("syncActions", self.transform, animation_player.current_animation, health, dieTime)
		

#sync actions of enemy instance
remote func syncActions(orientation, anim_state, lifePoints, deathClock):
